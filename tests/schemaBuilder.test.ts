...
import { buildSchema } from '../src/domain/schemaBuilder';
...
describe('SchemaBuilder.build — scalar node', () => {
  it('returns { type } for a non-object, non-array node', () => {
    addNode('root', 'string');
    assert.deepEqual(buildSchema(State.nodes, State.edges,'root'), { type: 'string' });
  });
...
describe('SchemaBuilder.build — missing root', () => {
  it('returns {} for an unknown nodeId', () => {
    assert.deepEqual(buildSchema(State.nodes, State.edges,'nonexistent'), {});
  });
});
